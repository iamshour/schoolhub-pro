...
const SideNavbar = () => {
	...
	return (
		<aside className='bg-background z-10 hidden min-h-full w-44 flex-col border-r sm:flex'>
			<nav className='flex flex-col items-center gap-4 px-2 sm:py-5'>
				...
				{navLinks.map(({ icon: Icon, label, to }) => (
					<Tooltip content={label} key={to} side='right'>
						<NavLink
							className={twMerge(
								"flex w-full items-center gap-2.5 rounded-lg border border-dashed bg-transparent p-2 px-2.5 text-gray-500 transition-basic hover:border-blue-50 hover:bg-blue-50 hover:text-blue-900",
